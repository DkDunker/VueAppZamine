...
export default {
  ...
  methods: {
    ...mapActions("registroMarcacion", ["obtenerMarcacionPersonalDiario", "siguientePaginaMarcacionPersonalDiario","siguientePaginaBusquedaMarcacionPersonalDiario"]),
    onLoadMoreItemsRequested(args) {
      const listView = args.object;
      let self = this;
      if (this.getPersonalMarcacionDiarioCabecera.total > this.asistencias_local.length) {
          setTimeout(function () {
            console.log('Loading more items...');
            self.siguienteMarcacionPersonalDiarioLocal(listView)
          }, 0);
    ...
    async obtenerMarcacionPersonalDiarioLocal(){
      await this.obtenerMarcacionPersonalDiario();
      this.asistencias_local = this.asistencias_local.concat(this.getPersonalMarcacionDiario)
    },
    async siguienteMarcacionPersonalDiarioLocal(listView){
      if(this.filtros != ''){
        this.filtros.next_page_url = this.getPersonalMarcacionDiarioCabecera.next_page_url
        await this.siguientePaginaBusquedaMarcacionPersonalDiario(this.filtros)
      }else{
        await this.siguientePaginaMarcacionPersonalDiario(this.getPersonalMarcacionDiarioCabecera.next_page_url)
      }
      const items = this.getPersonalMarcacionDiario;
      this.asistencias_local.push(...items);
      listView.notifyAppendItemsOnDemandFinished(items.length);
    },
    search(filtros){
        this.filtros = filtros
        this.asistencias_local = this.getPersonalMarcacionDiario
        this.$refs.listViewAsistenciaDiaria.refresh();
    },
    onLabelLoaded(args) {
      if (isAndroid) {
  ...
  computed: {
      ...mapGetters("registroMarcacion", ["getPersonalMarcacionDiario", "getPersonalMarcacionDiarioCabecera"]),
  },
  mounted() {},
  created() {
    this.obtenerMarcacionPersonalDiarioLocal()
